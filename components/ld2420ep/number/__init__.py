...
LD2420EPTimeoutNumber = ld2420ep_ns.class_("LD2420EPTimeoutNumber", number.Number)
LD2420EPMoveSensFactorNumber = ld2420ep_ns.class_(
    "LD2420EPMoveSensFactorNumber", number.Number
)
LD2420EPStillSensFactorNumber = ld2420ep_ns.class_(
    "LD2420EPStillSensFactorNumber", number.Number
)
LD2420EPMinDistanceNumber = ld2420ep_ns.class_("LD2420EPMinDistanceNumber", number.Number)
LD2420EPMaxDistanceNumber = ld2420ep_ns.class_("LD2420EPMaxDistanceNumber", number.Number)
LD2420EPGateSelectNumber = ld2420ep_ns.class_("LD2420EPGateSelectNumber", number.Number)
LD2420EPMoveThresholdNumbers = ld2420ep_ns.class_(
    "LD2420EPMoveThresholdNumbers", number.Number
)
LD2420EPStillThresholdNumbers = ld2420ep_ns.class_(
    "LD2420EPStillThresholdNumbers", number.Number
)
...
async def to_code(config):
    ...
    if gate_move_sensitivity_config := config.get(CONF_GATE_MOVE_SENSITIVITY):
        ...
        await cg.register_parented(n, config[CONF_LD2420EP_ID])
        cg.add(LD2420EP_component.set_gate_move_sensitivity_factor_number(n))
    if gate_still_sensitivity_config := config.get(CONF_GATE_STILL_SENSITIVITY):
        ...
        await cg.register_parented(n, config[CONF_LD2420EP_ID])
        cg.add(LD2420EP_component.set_gate_still_sensitivity_factor_number(n))
    if config.get(CONF_GATE_SELECT):
        if gate_number := config.get(CONF_GATE_SELECT):
            ...
            await cg.register_parented(n, config[CONF_LD2420EP_ID])
            cg.add(LD2420EP_component.set_gate_select_number(n))
        if gate_still_threshold := config.get(CONF_STILL_THRESHOLD):
            ...
            await cg.register_parented(n, config[CONF_LD2420EP_ID])
            cg.add(LD2420EP_component.set_gate_still_threshold_numbers(0, n))
        if gate_move_threshold := config.get(CONF_MOVE_THRESHOLD):
            ...
            await cg.register_parented(n, config[CONF_LD2420EP_ID])
            cg.add(LD2420EP_component.set_gate_move_threshold_numbers(0, n))
    else:
        for x in range(16):
            if gate_conf := config.get(f"gate_{x}"):
                ...
                await cg.register_parented(n, config[CONF_LD2420EP_ID])
                cg.add(LD2420EP_component.set_gate_move_threshold_numbers(x, n))
                ...
                await cg.register_parented(n, config[CONF_LD2420EP_ID])
                cg.add(LD2420EP_component.set_gate_still_threshold_numbers(x, n))
